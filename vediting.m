...
trainedLanguage = '/home/exs/editing_vid/myLang/tessdata/myLang.traineddata';
%%%%%%%%%%%%% TO DO MANUALLY : FIND A FRAME CONTAINING THE VERSUS %%%%%%%%
%%%%%%%%%%%%% IN ORDER TO SELECT THE REGION OF INTEREST %%%%%%%%%%%%%%%%%%
biggestTextBoxStateFrame = '0000160.png';
basicImage = imread(fullfile(workingDir,allFramesDir,biggestTextBoxStateFrame));
figure ; imshow(basicImage);
h = imrect ; roi = wait(h);
croppedImage = imcrop(basicImage, roi);
...
for j = 1:numel(imageNames)
  ...
  tic
  ocrResults = ocr(img);
  t1 = toc ; disp(['OCR1 took ',num2str(t1),' sec'])
  
  disp(ocrResults.Text); %1xN char array
  tic
  ocrResults = ocr(img,'Language', trainedLanguage);
  t3 = toc ; disp(['OCR2 took ',num2str(t3),' sec'])
  disp(ocrResults.Text); %1xN char array
  
  tic
  ocrResults = ocr(img,roi,'Language', trainedLanguage);
  t4 = toc ; disp(['OCR3 took ',num2str(t4),' sec'])
  disp(ocrResults.Text); %1xN char array
